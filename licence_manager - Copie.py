...
class LicenceManagerFrame(Frame):
    ...
    def register_credit(self):
        """
        Enregistre un crédit (ajustement) qui RÉDUIT la dette de l'agence,
        distinct d'un paiement. Requiert un accès admin (SECRET).
        """
        try:
            agency = self.agency_var.get()
            if not agency:
                self.result_label.config(text="⚠️ Sélectionnez une agence.")
                return

            amount = float(self.credit_var.get())
            if amount <= 0:
                self.result_label.config(text="⚠️ Entrez un montant de crédit valide (> 0).")
                return

            description = (self.credit_reason_var.get() or "").strip()

            headers = {"Authorization": f"Bearer {SECRET}"}
            payload = {
                "agency_name": agency,
                "amount": amount,
                "description": description,  # côté Flask, on l’acceptera en option
            }

            response = requests.post(API_URL_REGISTER_CREDIT, json=payload, headers=headers)
            result = response.json()

            if result.get("success"):
                new_debt = result.get("new_debt")
                if isinstance(new_debt, (int, float)):
                    self.result_label.config(
                        text=f"✅ Crédit enregistré. Nouvelle dette : {self._format_balance_for_display(result.get('new_debt', 0))}"
                    )
                else:
                    self.result_label.config(text="✅ Crédit enregistré.")
                self.refresh_debt_display()

                # Log local : on emploie la même logique que pour le paiement (montant négatif)
                try:
                    db_logger.log(agency, "credit", description, -amount)
                except Exception:
                    pass  # ne bloque pas l'UI si le logger local échoue

                # Nettoyage des champs
                self.credit_var.set("")
                self.credit_reason_var.set("")
            else:
                self.result_label.config(text=f"❌ Erreur: {result.get('error')}")
        except ValueError:
            self.result_label.config(text="⚠️ Montant de crédit invalide.")
        except Exception as e:
            # `response` peut ne pas exister si la requête plante avant
    ...
    def _format_balance_for_display(self, amount: float) -> str:
        """
        Convention visuelle:
          - Dette (>= 0):  "123.45 $"
          - Crédit (< 0):  "123.45 $ CR"
    ...
    def refresh_debt_display(self, *_):
        ...
        try:
            ...
            if result.get("success"):
                ...
                if debt < 0:
                    self.debt_label.config(fg="green")  # solde créditeur
                elif debt > 0:
                    self.debt_label.config(fg="red")  # dette
                else:
                    self.debt_label.config(fg="black")  # neutre
    ...
    def add_work(self):
        try:
            ...
            result = response.json()
            if result.get("success"):
                self.result_label.config(text=f"✅ Travail ajouté. Nouvelle dette : {self._format_balance_for_display(result.get('new_debt', 0))}")
                self.refresh_debt_display()
